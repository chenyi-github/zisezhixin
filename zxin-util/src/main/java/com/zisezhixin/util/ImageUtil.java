...
public class ImageUtil {
     ...
     * @param force
     */
    public static void thumbnailImage(String srcimagePath, int w, int h,
     ...
     * @param force
     */
    public static void thumbnailImage_s(String imagePath, int w, int h,
            String dstFilePath, boolean force) {
     ...
     * @param force
     */
    public static void thumbnailImage_m(String imagePath, int w, int h,
            String dstFilePath, boolean force) {
        thumbnailImage(imagePath, w, h, dstFilePath, force);
    }
    ...
    public static String uploadImage(MultipartFile multipartFile,
            StringBuilder path, StringBuilder fileNameSb,
            StringBuilder extNameSb) throws IOException {
        String status = "";
        if (multipartFile != null) {
            ...
            String extName = fileName.substring(fileName.lastIndexOf(".") + 1)
                    .toLowerCase(); // 得到文件扩展名，并统一转换成小写
            extNameSb.append(extName);
            
            InputStream is = multipartFile.getInputStream();
            try {
                ImageIO.read(is); // 用于获得文件宽和高用
            } catch (Exception e) {
                status = "-1"; // 图片格式有问题
            }
            ...
            String fieId = ImageUtil.uploadImageToServer(
                    multipartFile.getInputStream(), extName, pictSize); // 上传图片
     ...
     * @throws IOException
     */
    private static String uploadImageToServer(InputStream inStream,
            String extName, long pictSize) throws IOException {
        String fileId = "";
        if (inStream == null) {
            return fileId;
        } else {
